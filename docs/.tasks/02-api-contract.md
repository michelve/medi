...
| `GET /api/files/:file_id` | A file's audio + subtitle tracks | `{ file_id, audio: [{ stream_index, codec?, channels?, channel_layout?, language?, title?, is_default }], subtitles: [{ id, stream_index?, external, format, language?, title?, is_default, is_forced }] }`. Lets a deep link to `/play/:id` (no router state) populate the player's audio/caption menus (`97-web-player-shell-and-controls.md` Part C; extended with chapters by `99`). |
