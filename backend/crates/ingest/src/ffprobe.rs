...
use medi_db::writes::MediaFileWrite;
...
/// Run `ffprobe` on `path` and parse its output into a [`MediaFileWrite`] ready to
/// persist via `medi_db::writes::upsert_media_file`.
///
...
pub async fn probe(path: &Path) -> Result<MediaFileWrite, ProbeError> {
...
/// Convert the parsed ffprobe JSON into the persistable write struct. Returns `None`
/// if the file has no video stream (nothing meaningful to catalog).
///
...
pub(crate) fn map_output(out: &FfprobeOutput) -> Option<MediaFileWrite> {
    let video = out
        .streams
        .iter()
...
mod tests {
    ...
    fn parse(json: &str) -> MediaFileWrite {
        ...
        map_output(&out).expect("has a video stream")
    }
